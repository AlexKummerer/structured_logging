...
async def benchmark_context_propagation(iterations: int = 2000):
    ...
    print("Sync Context Logging:")
    print(f"  Mean time: {sync_stats['mean_ms']:.3f}ms per log")
    print(f"  Throughput: {sync_stats['throughput_per_sec']:.0f} logs/second")

    print("Async Context Logging:")
    print(f"  Mean time: {async_stats['mean_ms']:.3f}ms per log")
    print(f"  Throughput: {async_stats['throughput_per_sec']:.0f} logs/second")
...
async def benchmark_concurrent_logging(
    concurrent_tasks: int = 10, logs_per_task: int = 100
):
...
async def benchmark_batch_sizes():
    ...
    for batch_size in batch_sizes:
        ...
        async_logger = get_async_logger(
            f"bench_batch_{batch_size}", async_config=async_config
        )
        async_logger.processor.stream = suppress_output().__enter__()

        async def batch_log():
            await async_logger.ainfo("Batch test message")
        ...
        print(
            f"Batch size {batch_size:3d}: {stats['throughput_per_sec']:8.0f} logs/sec "
    ...
    print(
        f"Optimal batch size: {optimal_batch} ({results[optimal_batch]['throughput_per_sec']:.0f} logs/sec)"
    )
...
async def benchmark_formatter_types_async():
    ...
    for formatter_type in formatters:
        ...
        print(
            f"{formatter_type.upper():<5}: {stats['throughput_per_sec']:8.0f} logs/sec "
            f"({stats['mean_ms']:.3f}ms per log)"
        )
...
async def benchmark_memory_usage():
    ...
    async with async_request_context(user_id="memory_test_user"):
        for i in range(log_count):
            await async_logger.ainfo(
                f"Memory test log {i}", batch=i // 100, data={"key": f"value_{i}"}
            )
...
async def benchmark_queue_performance():
    ...
    for queue_size in queue_sizes:
        ...
        async_logger = get_async_logger(
            f"bench_queue_{queue_size}", async_config=async_config
        )
        async_logger.processor.stream = suppress_output().__enter__()
        ...
        print(
            f"Queue size {queue_size:4d}: {queue_time:.3f}s to queue {test_load:,} logs "
