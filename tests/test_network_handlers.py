...
class TestSocketConfig:
    ...
    def test_udp_socket_config(self):
        config = SocketConfig(protocol="udp", port=9999, udp_buffer_size=32768)
...
class TestSyslogHandler:
    ...
    def test_rfc3164_formatting(self):
        config = SyslogConfig(facility=16, app_name="test-app", hostname="test-host")
        handler = SyslogHandler(config)
        ...
        record.created = time.mktime(
            time.strptime("2024-01-15 10:30:00", "%Y-%m-%d %H:%M:%S")
        )
    ...
    def test_rfc5424_formatting(self):
        config = SyslogConfig(
            facility=16, app_name="test-app", hostname="test-host", rfc_format="5424"
        )
        handler = SyslogHandler(config)
        handler.hostname = "test-host"
...
class TestHTTPHandler:
    ...
    def test_auth_setup_api_key(self):
        config = HTTPConfig(
            auth_type="api_key", api_key="key123", api_key_header="X-Custom-Key"
        )
        handler = HTTPHandler(config)
    ...
    def test_auth_setup_basic(self):
        config = HTTPConfig(auth_type="basic", username="user", password="pass")
        handler = HTTPHandler(config)
    ...
    def test_http_single_message_send(self, mock_urlopen):
        ...
        config = HTTPConfig(url="https://api.test.com/logs", batch_size=1)
        handler = HTTPHandler(config)

        record = logging.LogRecord("test", logging.INFO, "", 0, "Test HTTP", (), None)
        messages = [
            {"message": "Test HTTP", "record": record, "timestamp": time.time()}
        ]
    ...
    def test_http_batch_send(self, mock_urlopen):
        ...
        config = HTTPConfig(url="https://api.test.com/logs", batch_size=2)
        handler = HTTPHandler(config)
    ...
    def test_http_retry_on_failure(self, mock_urlopen):
        ...
        messages = [
            {"message": "Test retry", "record": record, "timestamp": time.time()}
        ]
...
class TestSocketHandler:
    ...
    def test_udp_size_limit(self):
        ...
        with patch("socket.socket") as mock_socket_class:
            ...
            record = logging.LogRecord(
                "test", logging.INFO, "", 0, large_message, (), None
            )
...
class TestNetworkLoggerIntegration:
    ...
    def test_end_to_end_network_logging(self, mock_socket_class):
        ...
        config = LoggerConfig(
            output_type="network", network_config=syslog_config, formatter_type="json"
        )
...
class TestNetworkHandlerPerformance:
    ...
    def test_high_volume_logging(self, mock_socket_class):
        ...
        for i in range(1000):
            record = logging.LogRecord(
                "test", logging.INFO, "", 0, f"Message {i}", (), None
            )
...
class TestNetworkHandlerErrorHandling:
    ...
    def test_fallback_handler_creation(self):
        ...
        config = SyslogConfig(
            fallback_to_local=True, local_fallback_file="test_fallback.log"
        )
        handler = SyslogHandler(config)
    ...
    def test_fallback_on_network_failure(self, mock_socket_class):
        ...
        record = logging.LogRecord(
            "test", logging.ERROR, "", 0, "Failed message", (), None
        )
    ...
    def test_buffer_overflow_handling(self):
        ...
        for i in range(5):
            record = logging.LogRecord(
                "test", logging.INFO, "", 0, f"Overflow {i}", (), None
            )
            handler.emit(record)
    ...
    def test_invalid_host_handling(self):
        ...
        record = logging.LogRecord(
            "test", logging.ERROR, "", 0, "Test invalid host", (), None
        )
