...
from structured_logging.performance import fast_timestamp
...
class TestSyncLoggingPerformance:
    ...
    def test_structured_logging_throughput(self):
        ...
        with patch("sys.stdout", new_callable=io.StringIO) as mock_stdout:
            ...
            assert (
                throughput >= 3000
            ), f"Structured throughput {throughput:.0f} < 3,000 logs/sec"

    def test_filtering_performance_impact(self):
        """Test performance impact of filtering"""
        # Test without filtering
        config_no_filter = LoggerConfig(output_type="console", formatter_type="json")
        ...
        overhead_percentage = (
            (with_filter_duration - no_filter_duration) / no_filter_duration
        ) * 100
        ...
        ), f"Filtering overhead {overhead_percentage:.1f}% > 50%"

        # Should still achieve reasonable throughput with filtering
        assert (
            with_filter_throughput >= 3000
        ), f"Filtered throughput {with_filter_throughput:.0f} < 3,000 logs/sec"
    ...
    def test_concurrent_logging_performance(self):
        ...
        assert (
            total_throughput >= 3000
        ), f"Concurrent throughput {total_throughput:.0f} < 3,000 logs/sec"
...
class TestAsyncLoggingPerformance:
    ...
    async def test_async_logging_throughput(self):
        ...
        async_config = AsyncLoggerConfig(
            queue_size=10000, batch_size=100, flush_interval=0.1, max_workers=2
        )
        ...
        assert (
            throughput >= 10000
        ), f"Async throughput {throughput:.0f} < 10,000 logs/sec"
    ...
    async def test_async_concurrent_performance(self):
        ...
        async_config = AsyncLoggerConfig(
            queue_size=20000, batch_size=200, flush_interval=0.1, max_workers=4
        )
        ...
        async def log_coroutine(coro_id: int, num_logs: int) -> None:
            ...
            logger = get_async_logger(
                f"async_concurrent_{coro_id}", config, async_config
            )

            for i in range(num_logs):
                await logger.ainfo(
                    f"Coroutine {coro_id} log {i}", coro_id=coro_id, log_id=i
                )
        ...
        assert (
            throughput >= 8000
        ), f"Async concurrent throughput {throughput:.0f} < 8,000 logs/sec"
...
class TestComponentPerformance:
    ...
    def test_timestamp_generation_performance(self):
        ...
        assert (
            fast_throughput > std_throughput
        ), "Fast timestamp should be faster than standard"
    ...
    def test_context_access_performance(self):
        ...
        assert (
            throughput >= 100000
        ), f"Context access {throughput:.0f} < 100,000 ops/sec"
    ...
    def test_filter_evaluation_performance(self):
        ...
        with patch("random.random", return_value=0.5):  # Consistent sampling
            start_time = time.perf_counter()
            for _ in range(iterations):
                engine.should_log(record, context)
            end_time = time.perf_counter()
        ...
        assert (
            throughput >= 40000
        ), f"Filter evaluation {throughput:.0f} < 40,000 ops/sec"
...
class TestMemoryUsage:
    ...
    def test_memory_efficiency(self):
        ...
        try:
            ...
            assert (
                memory_increase < 100
            ), f"Memory increase {memory_increase:.1f} MB > 100 MB"
...
class TestPerformanceRegression:
    ...
    def test_baseline_performance_requirements(self):
        ...
        filter_config = FilterConfig(
            enabled=True, filters=[LevelFilter(min_level=logging.INFO)]
        )
        config = LoggerConfig(
            output_type="console", formatter_type="json", filter_config=filter_config
        )
        ...
        print(
            f"  Structured + filtering: {structured_throughput:,.0f} logs/sec (required: 3,000)"
        )

        # Verify requirements
        assert (
            basic_throughput >= 20000
        ), f"Basic logging {basic_throughput:.0f} < 20,000 logs/sec"
        assert (
            structured_throughput >= 3000
        ), f"Structured logging {structured_throughput:.0f} < 3,000 logs/sec"
