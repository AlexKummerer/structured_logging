...
class TestFastAPIIntegration:
    ...
    def test_basic_fastapi_integration(self):
        ...
        # Check request log
        request_log = next((log for log in logs if "Request started" in log.get("message", "")), None)
        assert request_log is not None
        assert request_log["method"] == "GET"
        assert request_log["path"] == "/test"
        
        # Check response log
        response_log = next((log for log in logs if "Request completed" in log.get("message", "")), None)
    ...
    def test_path_exclusion(self):
        ...
        normal_logs = [log for log in request_logs if "/normal" in log.get("message", "")]
        health_logs = [log for log in request_logs if "/health" in log.get("message", "")]
    ...
    def test_minimum_duration_filtering(self):
        ...
        slow_logs = [log for log in completion_logs if "/slow" in log.get("message", "")]
        fast_logs = [log for log in completion_logs if "/fast" in log.get("message", "")]
...
class TestEdgeCases:
    ...
    def test_middleware_with_exception_in_logging(self):
        ...
        logging_config = FastAPILoggingConfig(
            logger_config=LoggerConfig(output_type="file", file_config=None)  # Invalid config
        )
    ...
    def test_request_body_size_limiting(self):
        ...
        try:
            ...
            if request_log and "request_body" in request_log:
                ...
                assert "[truncated]" in body_str or len(body_str) <= 20  # Should be limited
