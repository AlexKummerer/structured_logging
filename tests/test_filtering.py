...
from structured_logging.logger import get_logger, log_with_context, get_filter_metrics, reset_filter_metrics
...
class TestContextFilter:
    def test_required_keys_filter(self):
        ...
        result = filter_obj.should_log(
            MagicMock(),
            {"user_id": "123", "request_id": "abc", "extra": "value"}
        )
        assert result.should_log is True
    ...
    def test_excluded_keys_filter(self):
        ...
        result = filter_obj.should_log(
            MagicMock(),
            {"user_id": "123", "debug_info": "secret"}
        )
        assert result.should_log is False
        assert "found excluded keys" in result.reason
        
    def test_key_value_filters(self):
        filter_obj = ContextFilter(key_value_filters={"env": "production", "service": "api"})
        
        # Should pass with matching values
        result = filter_obj.should_log(
            MagicMock(),
            {"env": "production", "service": "api", "user_id": "123"}
        )
        assert result.should_log is True
...
class TestFilterConfig:
    def test_production_config(self):
        config = FilterConfig.create_production_config(sample_rate=0.1, max_logs_per_second=500)
