...
class TestRotatingFileHandler:
    ...
    def test_rotation_on_size(self):
        ...
        for i in range(5):  # This should exceed 1024 bytes
            record = logging.LogRecord(
                name="test", level=logging.INFO, pathname="", lineno=0,
                msg=f"{large_message}_{i}", args=(), exc_info=None
            )
            handler.emit(record)
    ...
    def test_backup_count_limit(self):
        ...
        assert not os.path.exists(f"{self.log_file}.4")  # Should not exist due to backup_count=3
...
class TestTimedRotatingFileHandler:
    ...
    def test_time_based_rotation_trigger(self):
        ...
        with patch('time.time') as mock_time:
            ...
            record = logging.LogRecord(
                name="test", level=logging.INFO, pathname="", lineno=0,
                msg="Before rotation", args=(), exc_info=None
            )
            handler.emit(record)
            ...
            record2 = logging.LogRecord(
                name="test", level=logging.INFO, pathname="", lineno=0,
                msg="After rotation", args=(), exc_info=None
            )
    ...
    def test_different_time_intervals(self):
        ...
        for when, interval, expected_seconds in test_cases:
            handler = TimedRotatingFileHandler(self.config, when=when, interval=interval)
...
class TestCreateFileLogger:
    ...
    def test_create_timed_file_logger(self):
        logger = create_file_logger("test_timed_logger", self.config, handler_type="timed")
    ...
    def test_custom_formatter(self):
        ...
        logger = create_file_logger("test_custom", self.config, formatter=custom_formatter)
