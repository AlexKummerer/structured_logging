...
class TestPerformanceOptimizations:
    ...
    def test_fast_timestamp_performance(self):
        ...
        assert (
            fast_stats["throughput_per_sec"] > regular_stats["throughput_per_sec"] * 1.2
        )
    ...
    def test_formatter_caching_works(self):
        ...
        get_logger("test_cache_3", config2)
    ...
    def test_formatter_type_performance_comparison(self):
        ...
        for fmt, stats in results.items():
            assert (
                stats["mean_ms"] < 0.5
            ), f"{fmt} formatter too slow: {stats['mean_ms']:.3f}ms"
            assert (
                stats["throughput_per_sec"] > 2000
            ), f"{fmt} formatter throughput too low"
        ...
        for fmt, stats in results.items():
            assert (
                stats["mean_ms"] < 0.1
            ), f"{fmt} formatter should be under 0.1ms, got {stats['mean_ms']:.3f}ms"

    def test_timestamp_caching_effectiveness(self):
        """Test that timestamp caching reduces overhead"""
        from structured_logging.performance import (
            create_optimized_timestamp_func,
        )
    ...
    def test_memory_efficiency(self):
        ...
        with request_context(user_id="user123", tenant_id="tenant456"):
            for i in range(1000):
                log_with_context(
                    logger, "info", f"Message {i}", iteration=i, batch=f"batch_{i//100}"
                )
        ...
        ), f"Memory usage too high: {current / 1024 / 1024:.1f}MB"
    ...
    def test_performance_regression_prevention(self):
        ...
        scenarios = [
            ...
            (
                "with_context",
                lambda: log_with_context(logger, "info", "Test", field="value"),
            ),
            (
                "complex_context",
                lambda: log_with_context(
                    logger,
                    "info",
                    "Test",
                    field1="value1",
        ...
        for scenario_name, func in scenarios:
            ...
            max_time_ms = {"basic": 0.05, "with_context": 0.2, "complex_context": 0.3}
            ...
            assert (
                stats["mean_ms"] < max_time_ms[scenario_name]
            ), f"{scenario_name} too slow: {stats['mean_ms']:.3f}ms"

            assert (
                stats["throughput_per_sec"] > min_throughput[scenario_name]
...
class TestPerformanceMeasurement:
    ...
    def test_performance_under_concurrent_load(self):
        ...
        assert (
            avg_time_per_log < 0.01
        ), f"Concurrent performance too slow: {avg_time_per_log*1000:.3f}ms per log"
