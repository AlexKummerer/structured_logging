...
from structured_logging import (
    get_logger, log_with_context, LoggerConfig,
    FilterConfig, LevelFilter, SamplingFilter
)
...
def benchmark_structured_logging(iterations=5000):
    ...
    with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
        ...
        start_time = time.perf_counter()
        for i in range(iterations):
            log_with_context(logger, "info", f"Structured message {i}", config,
                           test_id=i, benchmark="structured")
        duration = time.perf_counter() - start_time
...
def benchmark_filtered_logging(iterations=3000):
    ...
    with patch('sys.stdout', new_callable=StringIO) as mock_stdout:
        with patch('random.random', return_value=0.5):  # Consistent sampling
            ...
            start_time = time.perf_counter()
            for i in range(iterations):
                log_with_context(logger, "info", f"Filtered message {i}", config,
                               test_id=i, benchmark="filtered")
            duration = time.perf_counter() - start_time
    ...
    def worker(worker_id):
        with patch('sys.stdout', new_callable=StringIO):
            ...
            for i in range(logs_per_worker):
                log_with_context(logger, "info", f"Concurrent message {i}", config,
...
def print_summary(results):
    ...
    checks = [
        ...
        ("Structured Logging ≥ 5,000 logs/sec", structured_result['throughput'] >= 5000),
...
def main():
    ...
    parser.add_argument("--compare", type=str, help="Compare with previous results from JSON file")
    ...
    if args.compare:
        try:
            ...
            for current in results:
                ...
                previous_result = next((r for r in previous['results'] if r['test'] == test_name), None)
