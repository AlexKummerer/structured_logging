...
def benchmark_context_logging(iterations: int = 10000):
    ...
    with capture_performance():
        with request_context(user_id="user123", tenant_id="tenant456"):
            for i in range(iterations):
                log_with_context(logger, "info", f"Context message {i}", 
                               request_number=i, batch_id=f"batch_{i//100}")
...
def benchmark_formatter_types(iterations: int = 5000):
    ...
    for formatter_type in formatters:
        ...
        with capture_performance():
            with request_context(user_id="user123", service=f"{formatter_type}_service"):
                for i in range(iterations):
                    log_with_context(logger, "info", f"Message {i}", 
                                   iteration=i, formatter=formatter_type)
...
def memory_benchmark():
    ...
    with request_context(user_id="user123", tenant_id="tenant456"):
        for i in range(1000):
            log_with_context(logger, "info", f"Memory test {i}", 
