...
from .context import request_context, set_custom_context
...
class FastAPILoggingConfig:
    ...
    # Filtering
    exclude_paths: Set[str] = field(default_factory=lambda: {"/health", "/metrics", "/favicon.ico"})
    ...
    sensitive_query_params: Set[str] = field(default_factory=lambda: {
...
class FastAPILoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def _extract_request_info(self, request: Request) -> Dict[str, Any]:
        ...
        if self.config.mask_sensitive_data and info.get("query_params"):
            info["query_params"] = self._filter_sensitive_query_params(info["query_params"])
    ...
    def _should_log_request(self, response: Optional[Response], duration_ms: float, exception_occurred: bool) -> bool:
    ...
    def _get_log_level_for_response(self, response: Optional[Response], exception_occurred: bool) -> str:
...
def add_structured_logging(
    app: FastAPI, 
    config: Optional[FastAPILoggingConfig] = None
) -> FastAPI:
    """
