...
from .async_context import aget_request_id, aget_user_context, aget_custom_context
        ...
        try:
            while self.running:
                try:
                    ...
                    should_flush = (
                        len(self.batch) >= self.async_config.batch_size or
                        time.time() - self.last_flush_time >= self.async_config.flush_interval
                    )
