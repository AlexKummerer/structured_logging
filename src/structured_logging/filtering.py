...
class LogFilter(ABC):
    ...
    @abstractmethod
    def should_log(self, record: logging.LogRecord, context: Dict[str, Any]) -> FilterResult:
...
class ContextFilter(LogFilter):
    ...
    def should_log(self, record: logging.LogRecord, context: Dict[str, Any]) -> FilterResult:
        ...
        if self.excluded_keys and any(key in context for key in self.excluded_keys):
            ...
            return FilterResult(
                should_log=False,
                reason=f"context_filter: found excluded keys {found}"
            )
...
class CustomFilter(LogFilter):
    ...
    def __init__(self, filter_func: Callable[[logging.LogRecord, Dict[str, Any]], bool],
                 name: str = "custom"):
...
class SamplingFilter(LogFilter):
    ...
    def _apply_rate_limiting(self, record: logging.LogRecord, context: Dict[str, Any]) -> FilterResult:
        ...
        if record.levelno >= logging.ERROR and len(times) < self.max_per_second + self.burst_allowance:
            times.append(now)
            return FilterResult(
                should_log=True, 
    ...
    def _random_sampling(self, record: logging.LogRecord, context: Dict[str, Any]) -> FilterResult:
    ...
    def _hash_sampling(self, record: logging.LogRecord, context: Dict[str, Any]) -> FilterResult:
    ...
    def _level_based_sampling(self, record: logging.LogRecord, context: Dict[str, Any]) -> FilterResult:
...
class FilterConfig:
    ...
    @classmethod
    def create_production_config(cls, sample_rate: float = 0.1, max_logs_per_second: int = 1000) -> "FilterConfig":
...
class FilterEngine:
    ...
    def __init__(self, config: FilterConfig):
        ...
        self._filter_stats: Dict[str, Dict[str, int]] = defaultdict(lambda: defaultdict(int))
