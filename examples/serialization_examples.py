...
def example_dataclass_logging():
    ...
    order = Order(
        ...
        metadata={"source": "web", "campaign": "summer_sale", "discount_applied": True},
    )
    ...
    with request_context(user_id=str(user.id), tenant_id="company-123"):
        ...
        logger.info(
            "User activity", user=user, action="purchase", timestamp=datetime.now()
        )
        ...
        logger.warning(
            ...
            order_summary={
                ...
                "shipping_address": {"country": "US", "expedited": True},
            },
        )
...
def example_custom_type_registration():
    ...
    def serialize_bank_account(account_obj, config):
        ...
        return {
            ...
            "balance": serialize_for_logging(
                account_obj.balance, config
            ),  # Use Money serializer
