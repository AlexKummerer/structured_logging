...
def example_performance_comparison():
    ...
    with request_context(test_type="lazy"):
        for i, record in enumerate(complex_records):
            log_with_context(
                logger_lazy, "info", f"Processing record {i}",
                record_data=record,
                processing_time=time.perf_counter()
            )
...
def example_memory_efficiency():
    ...
    print(f"🔄 Processing only first 10% of objects...")
    ...
    print(f"📊 Final serialization stats:")
...
def example_conditional_serialization():
    ...
    test_scenarios = [
        ...
        ("User action", {"user_id": str(uuid4()), "action": "login", "timestamp": datetime.now()}),
        ("Complex transaction", {
            f"item_{i}": {
                "id": str(uuid4()),
                "data": "x" * 100,
        ...
        ("Debug dump", {f"debug_info_{i}": f"detail_{i}" * 50 for i in range(100)})  # Very large
    ]
...
def example_advanced_lazy_patterns():
    ...
    nested_structure = {
        "level1": create_lazy_serializable({
            "level2": create_lazy_serializable({
                "level3": {"deep_data": "x" * 1000}
            })
        }),
        "parallel_branch": create_lazy_serializable({
            "data": list(range(500))
        })
    }
    ...
    for i, data in enumerate(sample_data):
        ...
        size_info = f"len={len(data)}" if hasattr(data, '__len__') else f"str_len={len(str(data))}"
        
        print(f"   Item {i} ({data_type}, {size_info}): {'🚀 LAZY' if is_lazy else '⚡ IMMEDIATE'}")
