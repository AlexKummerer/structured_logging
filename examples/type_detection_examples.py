...
def example_smart_converter_intelligence():
    ...
    for name, data in test_cases:
        ...
        if isinstance(result, dict) and "_detection" in result:
            ...
            print(f"   ✨ {name}: Auto-detected as {detected_type} (confidence: {confidence:.2f})")
...
def example_performance_comparison():
    ...
    print(f"   Without detection: {time_no_detection:.3f}s ({time_no_detection/iterations*1000:.2f}ms per op)")
    print(f"   With detection:    {time_with_detection:.3f}s ({time_with_detection/iterations*1000:.2f}ms per op)")
