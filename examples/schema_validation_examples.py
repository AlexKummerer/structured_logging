...
def example_basic_schema_validation():
    ...
    user_schema = {
        ...
        "active": "bool",
        "tags": {"type": "list", "min_items": 0, "max_items": 10},
    }
    ...
    # Test invalid data
    invalid_users = [
        {**valid_user, "username": "jo"},  # Too short
        {**valid_user, "email": "invalid-email"},  # Invalid format
        {**valid_user, "age": 200},  # Too old
        {**valid_user, "tags": ["tag"] * 15},  # Too many tags
    ]
...
def example_validation_decorator():
    ...
    def create_user_account(
        username: str, email: str, age: int, preferences: dict
    ) -> dict:
    ...
    def update_user_profile(
        user_id: str, updates: Dict[str, Union[str, int, bool]]
    ) -> bool:
    ...
    try:
        ...
        updated = update_user_profile(
            user_id=user["user_id"], updates={"age": 29, "theme": "light"}
        )
...
def example_custom_validators():
    ...
    payment_schema = {
        "card_number": {"type": "str", "validator": validate_credit_card},
        "phone": {"type": "str", "validator": validate_phone_number},
        "password": {"type": "str", "validator": validate_password_strength},
        "amount": {"type": "float", "min_value": 0.01, "max_value": 10000.00},
    }
...
def example_validation_with_logging():
    ...
    api_request_schema = {
        "request_id": "str",
        "method": {"type": "str", "choices": ["GET", "POST", "PUT", "DELETE", "PATCH"]},
        "endpoint": "str",
        "user_id": {"type": "str", "required": False},
        "response_status": {"type": "int", "min_value": 100, "max_value": 599},
        "response_time_ms": {"type": "float", "min_value": 0},
    ...
    def log_api_request(request_data: dict) -> bool:
        ...
        try:
            ...
            with request_context(request_id=request_data["request_id"]):
                log_with_context(
                    logger, "info", "API request processed", **request_data
                )
            return True
...
def example_validation_performance():
    ...
    print(
        f"   Average per validation: {(total_time / len(test_activities)) * 1000:.2f}ms"
    )
    ...
    print(
        f"   Success rate: {((stats['validations_performed'] - stats['validation_failures']) / stats['validations_performed'] * 100):.1f}%"
    )
...
def example_validation_error_handling():
    ...
    complex_schema = {
        "id": {"type": "str", "pattern": r"^[A-Z]{3}_\d{6}$"},  # Format: ABC_123456
        "email": {
            "type": "str",
