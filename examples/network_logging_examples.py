...
def example_http_logging():
    ...
    http_config = HTTPConfig(
        ...
        headers={"X-Service": "demo-app", "X-Environment": "production"},
    )

    config = LoggerConfig(
        output_type="console+network", network_config=http_config, formatter_type="json"
    )
...
def example_socket_logging():
    ...
    config = LoggerConfig(
        output_type="console+network", network_config=tcp_config, formatter_type="json"
    )
...
def example_mixed_outputs():
    ...
    error_syslog_config = SyslogConfig(host="localhost", port=514, facility=16)
